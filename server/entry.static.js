...
const newInvokeContext = (hostElement, element, event, url2) => ({
  $seq$: 0,
...
let _platform = (() => {
  ...
  return {
    isServer: false,
    importSymbol(containerEl, url2, symbolName) {
      const urlDoc = ((doc, containerEl2, url3) => {
        var _a3;
        const baseURI = doc.baseURI;
        const base = new URL((_a3 = containerEl2.getAttribute("q:base")) != null ? _a3 : baseURI, baseURI);
        return new URL(url3, base);
      })(containerEl.ownerDocument, containerEl, url2).toString();
  ...
  for (let i2 = 0; i2 < attributes.length; i2++) {
    ...
    if (name.startsWith("on:") || name.startsWith("on-window:") || name.startsWith("on-document:")) {
      ...
      for (const url2 of urls) {
        const qrl = parseQRL(url2, containerEl);
...
async function createNodeMainProcess(opts) {
  ...
  const createWorker = () => {
    ...
    nodeWorker.on("error", (e2) => {
      console.error(`worker error: ${e2}`);
    });
...
async function createNodeWorkerProcess(onMessage) {
  var _a22;
  (_a22 = worker_threads.parentPort) == null ? void 0 : _a22.on("message", async (msg) => {
    var _a3;
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      const addToQueue = (pathname, params) => {
        ...
        if (pathname && !queue.some((s2) => s2.pathname === pathname)) {
          queue.push({
            pathname,
            params
          });
...
function validateOptions(opts) {
  ...
  try {
    ...
    throw new Error(`Invalid "origin": ${e2}`);
  }
}
...
var loadRoute$1 = async (routes3, menus2, cacheModules2, pathname) => {
...
var RedirectResponse = class {
  constructor(url2, status, headers) {
    this.url = url2;
    this.location = url2;
...
async function loadUserResponse(requestCtx, params, routeModules, platform, trailingSlash3, basePathname3 = "/") {
  ...
  const { request, url: url2 } = requestCtx;
  const { pathname } = url2;
  ...
  if (isPageModule && pathname !== basePathname3) {
    if (trailingSlash3) {
      if (!pathname.endsWith("/")) {
        throw new RedirectResponse(pathname + "/" + url2.search, 307);
      }
    } else {
      if (pathname.endsWith("/")) {
        throw new RedirectResponse(
          pathname.slice(0, pathname.length - 1) + url2.search,
          307
        );
      }
    }
  ...
  const redirect = (url22, status) => {
    return new RedirectResponse(url22, status, userResponse.headers);
  };
...
function getQwikCityEnvData(userResponse) {
  const { url: url2, params, pendingBody, resolvedBody, status } = userResponse;
  return {
    url: url2.href,
    qwikcity: {
      params: { ...params },
...
async function workerRender(sys, render2, opts, staticRoute, pendingPromises, callback) {
  const url2 = new URL(staticRoute.pathname, opts.origin);
  const result = {
    type: "render",
  ...
  try {
    ...
    requestHeaders.set("Host", url2.host);
    ...
    const promise = requestHandler(
      ...
      if (e2) {
        if (e2.stack) {
          result.error = String(e2.stack);
        } else if (e2.message) {
          result.error = String(e2.message);
        } else {
          result.error = String(e2);
        }
      } else {
        result.error = `Error`;
      }
...
function addBundle(manifest2, urls, prefetchResources, buildBase, bundleFileName) {
  const url2 = buildBase + bundleFileName;
  if (!urls.has(url2)) {
    urls.add(url2);
...
function workerFetchScript() {
  ...
  let s2 = `const w=new Worker(${url2});`;
...
const toPath = (url2) => url2.pathname + url2.search + url2.hash;
const toUrl = (url2, baseUrl) => new URL(url2, baseUrl.href);
...
const QwikCity = /* @__PURE__ */ componentQrl(inlinedQrl(() => {
  ...
  useWatchQrl(inlinedQrl(async ({ track }) => {
    ...
    const url22 = new URL(path2, routeLocation2.href);
    const pathname = url22.pathname;
    const loadRoutePromise = loadRoute(routes2, menus2, cacheModules2, pathname);
    const endpointResponse = isServer ? env2.response : loadClientData(url22.href);
    const loadedRoute = await loadRoutePromise;
    if (loadedRoute) {
      const [params, mods, menu] = loadedRoute;
      const contentModules = mods;
      const pageModule = contentModules[contentModules.length - 1];
      routeLocation2.href = url22.href;
      routeLocation2.pathname = pathname;
      routeLocation2.params = {
        ...params
      };
      routeLocation2.query = Object.fromEntries(url22.searchParams.entries());
